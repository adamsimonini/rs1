<template>
  <div class="map">
    <mapbox
      ...
      @map-load="loaded"
      @map-click:rs1-centroids-layer="centroidsLayerClicked"
      @map-mouseenter:rs1-centroids-layer="centroidsLayerMouseEntered"
      @map-mouseleave:rs1-centroids-layer="centroidsLayerMouseLeft"
    />
    <!-- <div id="map"></div> -->
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    loaded(map) {
      // Uncomment this block to add the live datasource from the hosted backend
      ...
      // Uncomment this block to add test data suitable for debugging (avoids CORS issues from requesting data from the live datasource from the hosted backend when running as localhost)
      // map.addSource('rs1-centroids', {
      //     "type": "geojson",
      //     "data": {
    ...
    centroidsLayerMouseEntered(map) {
      map.getCanvas().style.cursor = "pointer";
    },
    centroidsLayerMouseLeft(map) {
      map.getCanvas().style.cursor = "";
    },
    centroidsLayerClicked(map, e) {
      if (e.features) {
